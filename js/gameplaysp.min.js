...
function gamewin(ply) {
    ...
    if (cnt == 4) {
        ...
        if(ply==1)
        {
            ...
            swal("Winner Winner Chicken Dinner","Congrats you won the game!!!","success").then(()=>{
            let lblink="https://chokosta.herokuapp.com/";
            location.assign(lblink);});
        }
        else{
        swal("Better luck next time","Well played try another new game ","error").then(()=>{ let lblink="https://chokosta.herokuapp.com/";
            location.assign(lblink); });
